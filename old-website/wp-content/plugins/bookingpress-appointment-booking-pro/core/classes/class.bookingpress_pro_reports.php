...
if ( ! class_exists( 'bookingpress_pro_reports' ) ) {
	class bookingpress_pro_reports Extends BookingPress_Core {
		...
		function bookingpress_get_customer_report_charts_data_func(){
			...
			if(!empty($custom_filter_val) && !empty($custom_filter_val[0]) && !empty($custom_filter_val[1])){
		...
		function bookingpress_get_appointment_report_func() {
			...
			if ( ! empty( $bookingpress_search_data ) && ! empty( $bookingpress_search_data['staff_member_name'] ) ) {
				...
				if(empty($multi_staff_filter)){
					...
					$appointments_search_query          .= $multi_staff_filter;
				}
			...
			if ( ! empty( $bookingpress_appointments_data ) ) {
				...
				foreach ( $bookingpress_appointments_data as $get_appointment ) {
					...
					if(!empty($bookingpress_appointment_details)){
						...
						$bookingpress_tmp_appointment_data = apply_filters('bookingpress_appointment_add_report_view_field', $bookingpress_tmp_appointment_data, $get_appointment);
						...
						if($service_duration_unit != 'd') {
							if( !empty( $bookingpress_appointment ) && method_exists( $bookingpress_appointment, 'bookingpress_get_appointment_duration' ) ){
								...
								$bookingpress_appointment_end_datetime = $bookingpress_appointment_end_date . ' ' . $get_appointment['bookingpress_appointment_end_time'];

								if( $bookingpress_appointment_end_date > $get_appointment['bookingpress_appointment_date'] && '00:00:00' != $get_appointment['bookingpress_appointment_end_time'] ){
									...
									if( $end_hour >= 24 ){
										$bookingpress_appointment_end_datetime = date('Y-m-d H:i:s', strtotime( $bookingpress_appointment_date .' 00:00:00 +'.$end_hour.' hours '.$end_mins.' minutes '.$end_sec.' seconds' ) );
									} else {
										$bookingpress_appointment_end_datetime = $bookingpress_appointment_end_date.' '.$get_appointment['bookingpress_appointment_end_time'];
									}
								} else {
		...
		function bookingpress_reports_vue_methods_func() {
			...
			load_appointment_data( reset_pagination = false ){
				const vm = this
				if( true == reset_pagination ){
					this.appointment_current_page = 1;
				}
				var bookingpress_search_data = {'service_name': vm.appointment_search_service,'staff_member_name' :vm.appointment_search_staff};
			...
			change_appointment_report_filter( reset_pagination = false ){
				const vm = this;
				vm.load_appointment_charts()
				vm.load_appointment_data( reset_pagination )
			},		
