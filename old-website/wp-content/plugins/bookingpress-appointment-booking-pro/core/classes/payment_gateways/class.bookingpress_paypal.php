            ...
            if(!empty($bookingpress_final_payment_data['appointment_id'])){
                ...
                if(!empty($bookingpress_payment_id)){
                    ...
                    if($bookingpress_final_payable_amount != 0){
						...
						if(!empty($bookingpress_final_payable_amount) && $bookingpress_final_payable_amount > 0	){
				...
				if(!empty($entry_id)){
					...
					if (empty($auth_response)) {
						...
						if (!empty($auth_response->access_token)) {
							...
							$decimal_points = $BookingPress->bookingpress_get_settings('price_number_of_decimals', 'payment_setting');

							if( is_float( $bookingpress_final_payable_amount ) && !in_array( $currency_code, ['HUF','JPY', 'TWD'] ) ){
								$bookingpress_final_payable_amount = number_format( $bookingpress_final_payable_amount, $decimal_points, '.', '' );
							}
							...
							if (is_wp_error($response_return)) {
								...
								if( empty( $order_data['id'] ) ){
									$error_messages = $order_data['name'] . '
									';
									$message_details = $order_data['details'];
									foreach( $message_details as $error_data ){
										$error_messages .= ' - '. $error_data['description'] .' ';
									}
									$response['variant'] = 'error';
									$resposne['title'] = esc_html__( 'Error', 'bookingpress-appointment-booking' );
		   ...
		   if(!empty($bookingpress_transaction_id ) && !empty($bookingpress_refund_data['refund_type'])) {						   
			   try{
					...
					if($bookingpres_refund_type != 'full') {                    
						$bookingpres_refund_amount = $bookingpress_refund_data['refund_amount'] ? number_format($bookingpress_refund_data['refund_amount'], 2) : 0;
