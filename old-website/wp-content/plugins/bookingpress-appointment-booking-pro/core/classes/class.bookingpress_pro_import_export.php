							...
							if(isset($import_record_data[$i])){
								...
								if(!empty($single_import_record)){
									...
									*/
									...
									$old_appointment_id = intval($single_import_record['bookingpress_appointment_booking_id']);
									...
									$bookingpress_import_export->bookingpress_set_record_rel('appointment_id', $export_key, $import_id, $old_appointment_id, $last_import_id, 'appointment_id');
						...
						for($i=$detail_import_last_record; $i<$new_limit; $i++){
							if(isset($import_record_data[$i])){
								...
								if(!empty($single_import_record)){
									...
									$bookingpress_staff_speical_day_services_arr = !empty( $import_record_data[$i]['bookingpress_special_day_service_id'] ) ? explode(",",$import_record_data[$i]['bookingpress_special_day_service_id']) : '';
									$final_service_arr = array();
									if(!empty($bookingpress_staff_speical_day_services_arr)){
										foreach($bookingpress_staff_speical_day_services_arr as $serid){
											$final_service_arr[] = $bookingpress_import_export->bookingpress_get_record_rel('services',$export_key,$import_id,$serid);
										}                                                            
									}
									if(!empty($final_service_arr)){
										$single_import_record['bookingpress_special_day_service_id'] = implode(',',$final_service_arr);
									}
