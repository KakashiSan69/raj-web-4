...
if ( ! class_exists( 'bookingpress_pro_customize' ) ) {
	class bookingpress_pro_customize Extends BookingPress_Core {
		...
		function __construct() {
			...
			/* add_filter( 'bookingpress_datetime_selection_visibility', '__return_true'); */
			
			add_action( 'bookingpress_before_save_customize_other_settings_data', array( $this, 'bookingpress_before_save_customize_other_settings_data_func'),10);

			add_action( 'bookingpress_after_save_customize_settings', array( $this, 'bookingpress_after_save_customize_settings_datetime_arrangement_func' ) );

			add_filter( 'bookingpress_before_save_customize_booking_form', array( $this, 'bookingpress_before_save_customize_booking_form_datetime_rearrangment_func') );
		}

		function bookingpress_before_save_customize_booking_form_datetime_rearrangment_func( $booking_form_settings_data ){

			$timeslot_styling = $booking_form_settings_data['booking_form_settings']['bookigpress_time_format_for_booking_form'];

			$form_sequence_arr = json_decode( $booking_form_settings_data['booking_form_settings']['bookingpress_form_sequance'], true );
			$service_step_pos = array_search( 'service_selection', $form_sequence_arr );
			$datetime_step_pos = array_search( 'datetime_selection', $form_sequence_arr );

			if( $datetime_step_pos < $service_step_pos ){
			
				if( 1 == $timeslot_styling || 5 == $timeslot_styling ){
					$booking_form_settings_data['booking_form_settings']['bookigpress_time_format_for_booking_form'] = 3;
				} else if( 2 == $timeslot_styling || 6 == $timeslot_styling ){
					$booking_form_settings_data['booking_form_settings']['bookigpress_time_format_for_booking_form'] = 4;
				}	
		...
		function bookingpress_after_save_customize_settings_datetime_arrangement_func(){
			global $wpdb, $BookingPress;

			$form_sequence_settings = $BookingPress->bookingpress_get_customize_settings( 'bookingpress_form_sequance', 'booking_form' );

			$form_sequence_arr = json_decode( $form_sequence_settings, true );

			$service_step_pos = array_search( 'service_selection', $form_sequence_arr );
			$datetime_step_pos = array_search( 'datetime_selection', $form_sequence_arr );

			if( $datetime_step_pos < $service_step_pos ){
				$BookingPress->bookingpress_update_settings('show_time_as_per_service_duration', 'general_setting', 'false');
				$BookingPress->bookingpress_update_settings('disable_show_time_per_duration_option', 'general_setting', 'true' );
			} else {
				$BookingPress->bookingpress_update_settings('disable_show_time_per_duration_option', 'general_setting', 'false' );
			}
		}

		function bookingpress_before_save_customize_other_settings_data_func( ){
			?>
			
			if( vm.booking_form_settings.bookingpress_form_sequance[0] == "datetime_selection" ){
				bpa_validation_flag = true;
			}
			<?php
		}
		...
		*/
		...
		function bookingpress_modify_bookingpress_form_sequance_data( $bookingpress_form_data ){
			...
			if(!empty($bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox'])) {					
				$bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox'] = explode(",",$bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox']);

				$bookingpress_selected_searchbox_data = $bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox'];                    

				$bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox'] = $bookingpress_selected_searchbox_data;
				

			} else  {
				$bookingpress_form_data['booking_form_settings']['bookingpress_selected_searchbox'] = array();
			}
			...
			if( count( $booking_form_sequence ) > 1 ){
				global $bookingpress_pro_staff_members;
				if( $bookingpress_pro_staff_members->bookingpress_check_staffmember_module_activation() && !in_array( 'datetime_selection', $booking_form_sequence ) ){
					array_push( $booking_form_sequence, 'datetime_selection' );
					//array_push( $bookingpress_form_data['booking_form_settings']['bookingpress_form_sequance'], 'staff_selection' );
			...
			$bookingpress_is_staffmember_module_activated = $bookingpress_pro_staff_members->bookingpress_check_staffmember_module_activation();
			$bookingpress_is_location_add_on_activated = is_plugin_active( 'bookingpress-location/bookingpress-location.php' ) ? true : false;
			...
			foreach( $booking_form_sequence as $key => $sequence ){
				...
				if( 'datetime_selection' == $sequence && (1 == $bookingpress_is_staffmember_module_activated || true == $bookingpress_is_location_add_on_activated) ){
					$sequence_visibility = true;
				}
		...
		function bookingpress_add_booking_form_customize_data_func() {
			...
			if( vm2.is_staffmember_activated == 1 && (vm2.booking_form_settings.bookingpress_form_sequance['0'] != 'undefined' && vm2.booking_form_settings.bookingpress_form_sequance['0'] == 'service_selection' && vm2.booking_form_settings.bookingpress_form_sequance['1'] == 'datetime_selection' && vm2.booking_form_settings.bookingpress_form_sequance['2'] == 'staff_selection') ) {
				vm2.hide_staff_step_disabled = true;	
			} else {
				vm2.hide_staff_step_disabled = false;	
			}
			<?php
		}
		...
		function bookingpress_before_save_customize_booking_form_func($booking_form_settings){
			...
			if( !empty( $booking_form_settings['booking_form_settings']['bookingpress_selected_searchbox'] ) ){
				$booking_form_settings['booking_form_settings']['bookingpress_selected_searchbox'] = implode(",",$booking_form_settings['booking_form_settings']['bookingpress_selected_searchbox']);
		...
		function bookingpress_get_my_booking_customize_data_filter_func( $bookingpress_my_booking_field_settings ) {
			...
			$bookingpress_my_booking_field_settings['allow_customer_book_again'] = false;
			$bookingpress_my_booking_field_settings['book_again_page'] = '';
			$bookingpress_my_booking_field_settings['auth_field'] = 'email_address';
			...
			$bookingpress_my_booking_field_settings['login_form_phone_number_field_label'] = '';
			...
			$bookingpress_my_booking_field_settings['login_form_phone_number_field_placeholder'] = '';
			...
			$bookingpress_my_booking_field_settings['login_form_phone_number_required_field_label'] = '';
			...
			$bookingpress_my_booking_field_settings['change_password_form_title'] = '';
			...
			$bookingpress_my_booking_field_settings['book_again_button_title'] = '';
			...
			$bookingpress_my_booking_field_settings['cancellation_reason_title'] = '';						
		...
		function bookingpress_get_booking_form_customize_data_filter_func($booking_form_settings) {
			$booking_form_settings['booking_form_settings']['bookingpress_form_sequance'] = json_encode( array( 'service_selection', 'staff_selection','datetime_selection' ) );
			...
			$booking_form_settings['booking_form_settings']['bpa_enable_searchbox'] = false;
			$booking_form_settings['booking_form_settings']['bookingpress_selected_searchbox'] = [];
			$booking_form_settings['booking_form_settings']['book_appointemnt_service_search_text'] = esc_html__('Search Service','bookingpress-appointment-booking');
			$booking_form_settings['booking_form_settings']['book_appointemnt_staff_search_text'] = esc_html__('Search Staff Member','bookingpress-appointment-booking');
			...
			$booking_form_settings['front_label_edit_data']['bookingpress_popup_btn_title'] = '';
			...
			$booking_form_settings['tab_container_data']['fileupload_max_limit_reach_msg'] = __("You've reached the maximum file limit.", 'bookingpress-appointment-booking');
		...
		function bookingpress_modify_customize_data_fields_func( $bookingpress_customize_vue_data_fields ) {
			...
			$bpa_all_pages = [];
			foreach( $bookingpress_all_pages as $bookingpress_page ){
				$bpa_all_pages[] = [
					'post_title' => $bookingpress_page['post_title'],
					'ID' => $bookingpress_page['ID']
				];
			}
			$bookingpress_customize_vue_data_fields['bookingpress_all_global_pages'] = $bpa_all_pages;
			...
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['login_form_phone_number_required_field_label'] = '';			
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['login_form_phone_number_field_label'] = '';
			...
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['login_form_phone_number_field_placeholder'] = '';
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['login_form_remember_me_field_label'] = '';
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['change_password_form_title'] = '';
			...
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['book_again_button_title'] = '';
			...
			$bookingpress_customize_vue_data_fields['my_booking_field_settings']['cancellation_reason_title'] = '';
			...
			$bookingpress_customize_vue_data_fields['hide_staff_step_disabled'] = false;
			...
			$bookingpress_customize_vue_data_fields['booking_form_sequence'] = array('service_selection', 'staff_selection','datetime_selection');
			...
			$db_form_sequence = $BookingPress->bookingpress_get_customize_settings( 'bookingpress_form_sequance', 'booking_form');
			
			$db_form_sequence_arr = json_decode( $db_form_sequence, true );

			$ordered = [];
			$unordered = [];
			foreach ($db_form_sequence_arr as $tabName) {
				foreach ( $bookingpress_customize_vue_data_fields['bookingpress_form_sequance_arr'] as $key => $item) {
					if ($item['tab_name'] === $tabName) {
						$ordered[$key] = $item;
						unset($bookingpress_customize_vue_data_fields['bookingpress_form_sequance_arr'][$key]);
						break;
					}
				}
			}

			$unordered = $bookingpress_customize_vue_data_fields['bookingpress_form_sequance_arr'];

			$final_sequence_arr = array_merge($ordered, $unordered);

			$bookingpress_customize_vue_data_fields['bookingpress_form_sequance_arr'] = $final_sequence_arr;
			...
			$bookingpress_customize_vue_data_fields['bookingpress_search_data_list'] = array(
				array(
					'label'   => esc_html__('Service Step', 'bookingpress-appointment-booking'),
					'value'   => 'search_service_step',
				),
				
			);

			$bookingpress_customize_vue_data_fields['bookingpress_search_data_list'] = apply_filters("bookingpress_search_data_list_arr", $bookingpress_customize_vue_data_fields['bookingpress_search_data_list']);
						...
						if ( isset( $matches[1] ) && '' !== $matches[1] ) {
							...
							foreach ( $parent_field_options['inner_fields'] as $inK => $inv ) {
								if ( $inv['id'] == 'inner_field_' . $val['bookingpress_form_field_id'] ) {
									...
									}elseif ( 'signature' == $val['bookingpress_field_type'] ) {
										$inner_field_type = 'signature';
									} else {
		...
		function bookingpress_modify_form_field_data( $form_fields_data, $db_field_options ) {
			...
			} elseif( 'signature' == $db_field_options['bookingpress_field_type'] ) {
				$form_fields_data['field_type'] = 'signature';
			} else {
			...
			if ( $form_fields_data['field_type'] == 'File' ) {
				...
				if( !isset( $form_fields_data['field_options']['multiple_file_upload'] ) ){
					$form_fields_data['field_options']['multiple_file_upload'] = false;
				}

				if( !isset( $form_fields_data['field_options']['max_file_upload'] ) ){
					$form_fields_data['field_options']['max_file_upload'] = "null";
				}
			...
			if ( ! empty( $form_fields_data['field_options']['inner_fields'] ) ) {
				...
				foreach ( $form_fields_data['field_options']['inner_fields'] as $k => $v ) {
					...
					if ( $v['field_type'] == 'File' ) {
						...
						if( empty( $v['field_options']['multiple_file_upload'] ) ){
							$form_fields_data['field_options']['inner_fields'][ $k ]['field_options']['multiple_file_upload'] = false;
						}

						if( empty( $v['field_options']['max_file_upload'] ) ){
							$form_fields_data['field_options']['inner_fields'][ $k ]['field_options']['max_file_upload'] = "null";
						}
		...
		function bookingpress_modify_form_field_data_before_save_func( $form_fields_data, $posted_field_settings ) {
			...
			if( isset( $field_options['multiple_file_upload'] ) && ( 'true' === $field_options['multiple_file_upload'] || true === $field_options['multiple_file_upload'] || 1 === $field_options['multiple_file_upload'] ) ){
				$field_options['multiple_file_upload'] = true;
			} else {
				$field_options['multiple_file_upload'] = false;
			}
		...
		function bookingpress_pro_customize_dynamic_vue_methods() {
			...
			bookingpress_change_form_sequence(){
				...
				if(vm.is_staffmember_activated == 1 && (vm.booking_form_settings.bookingpress_form_sequance['0'] != 'undefined' && vm.booking_form_settings.bookingpress_form_sequance['0'] == "service_selection" && vm.booking_form_settings.bookingpress_form_sequance['1'] == "datetime_selection" && vm.booking_form_settings.bookingpress_form_sequance['2'] == "staff_selection" ) ){

					vm.booking_form_settings.hide_staffmember_selection = false;
					vm.hide_staff_step_disabled = true
					vm.bookingpress_form_sequance_arr['staffmembers']['is_visible'] = '1';
					
				}else{
					
					vm.hide_staff_step_disabled = false
				}
			...
			bpa_draggable_start: function(){
				const vm = this;
				
				vm.old_reference_seq = Object.assign( [], vm.booking_form_settings.bookingpress_form_sequance );
			},
			bpa_draggable_steps: function( event ){

				let formSequence_name = event.draggedContext.element;
				let newIndex = event.draggedContext.futureIndex;
				let oldIndex = event.draggedContext.index;
				
				if( formSequence_name == "datetime_selection" && newIndex == 0 ){
					return false;
				}

				<?php do_action("bookingpress_disable_step_for_drag_drop"); ?>
			...
			updateSequencePos: function(currentElement){
				...
				let old_sequance_arr = vm.booking_form_settings.bookingpress_form_sequance;
				...
				if( 'datetime_selection' == vm.booking_form_settings.bookingpress_form_sequance[0] ){
					let newSequencePos = {};
					vm.old_reference_seq.map( (item, index) =>{
						newSequencePos[ item ] = index + 1;	
					});
					vm.booking_form_settings.bpa_sequance_pos = Object.assign( {}, newSequencePos );
					vm.booking_form_settings.bookingpress_form_sequance = Object.assign( [], vm.old_reference_seq );
				}
				<?php do_action('bookingpress_change_position_outside_validation'); ?>
					...
					if(isset($form_fields_data['field_options']['inner_fields'][0]['field_options']['inner_fields']) && !empty($form_fields_data['field_options']['inner_fields'][0]['field_options']['inner_fields'])){
						...
						foreach ( $form_fields_data['field_options']['inner_fields'][0]['field_options']['inner_fields'] as $kk => $vv ) {
							...
							if ( $vv['field_type'] == 'File' ) {
								...
								if( empty( $vv['field_options']['multiple_file_upload'] ) ){
									$form_fields_data['field_options']['inner_fields'][0]['field_options']['inner_fields'][ $kk ]['field_options']['multiple_file_upload'] = false;
								}

								if( empty( $vv['field_options']['max_file_upload'] ) ){
									$form_fields_data['field_options']['inner_fields'][0]['field_options']['inner_fields'][ $kk ]['field_options']['max_file_upload'] = "null";
								}
