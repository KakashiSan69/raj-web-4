...
if ( ! class_exists( 'bookingpress_pro_email_notifications' ) ) {
	class bookingpress_pro_email_notifications Extends BookingPress_Core {
		...
		function __construct() {
			...
			add_filter('bookingpress_add_customer_cc_email_address', array($this, 'bookingpress_add_customer_cc_email_address_func'), 10, 3);
		...
		function bookingpress_add_customer_cc_email_address_func($bookingpress_cc_emails, $email_notification_name, $appointment_id = 0){
			global $wpdb, $BookingPress, $tbl_bookingpress_appointment_bookings, $tbl_bookingpress_staffmembers, $tbl_bookingpress_notifications, $tbl_bookingpress_guests_data, $tbl_bookingpress_form_fields, $bookingpress_pro_appointment;
			if(!empty($email_notification_name)){
				$email_notification_data = $wpdb->get_row( $wpdb->prepare( "SELECT `bookingpress_notification_cc_email` FROM {$tbl_bookingpress_notifications} WHERE bookingpress_notification_name = %s AND bookingpress_notification_receiver_type = 'customer' ORDER BY bookingpress_notification_id DESC", $email_notification_name ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_notifications is table name.

				if ( ! empty( $email_notification_data['bookingpress_notification_cc_email'] ) ) {
					$bookingpress_cc_emails = explode( ',', $email_notification_data['bookingpress_notification_cc_email'] );

					if(!empty($bookingpress_cc_emails)){
						if (!empty($appointment_id)) {
							$bookingpress_appointment_guest_key_data = array();
							$bookingpress_guest_details = $wpdb->get_results($wpdb->prepare( "SELECT * FROM ".$tbl_bookingpress_guests_data." Where bookingpress_guest_data_appointment_id = %d Order By bookingpress_guest_data_id ASC", $appointment_id), ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared --Reason: $tbl_bookingpress_guests_data is a table name. false alarm	
							...
							if(!empty($bookingpress_guest_details)){
								...
								if(!empty($bookingpress_appointment_guest_key_data)){
									...
									if(!empty($repeater_emails)){
										$bookingpress_cc_emails = array_merge($bookingpress_cc_emails, $repeater_emails);
									}
								}
							}	
						}

						$bookingpress_custom_fields_meta_values = $bookingpress_pro_appointment->bookingpress_get_appointment_form_field_data($appointment_id);
						if (!empty($bookingpress_custom_fields_meta_values)) {
							preg_match_all('/%([^%]+)%/', implode(',', $bookingpress_cc_emails), $matches);
							$placeholders = array_diff($matches[1] ?? [], ['customer_email']); // Exclude 'customer_email'
							if(!empty($placeholders)){
								foreach ($placeholders as $placeholder) {
									if (isset($bookingpress_custom_fields_meta_values[$placeholder]) && !empty($bookingpress_custom_fields_meta_values[$placeholder])){
										$bookingpress_cc_emails[] = $bookingpress_custom_fields_meta_values[$placeholder];
									}
								}
							}
						}						
						$bookingpress_cc_emails = array_filter($bookingpress_cc_emails, function($email) {
							$email = trim($email);
							return $email !== '%customer_email%' && filter_var($email, FILTER_VALIDATE_EMAIL);
						});

						$bookingpress_cc_emails = array_values(array_unique($bookingpress_cc_emails));	
					}
				}
			}
			return $bookingpress_cc_emails;
		...
		function bookingpress_modify_email_content_filter_func( $template_content, $bookingpress_appointment_data ) {
			...
			if(!empty($bookingpress_appointment_data)) {
				...
				$bookingpress_appointment_end_date   = ! empty( $bookingpress_appointment_data['bookingpress_appointment_end_date'] ) ? esc_html( $bookingpress_appointment_data['bookingpress_appointment_end_date'] ) : '';
				...
				$start_datetime_string = $bookingpress_appointment_date . ' ' . $bookingpress_appointment_start_time;
				$end_datetime_string = $bookingpress_appointment_end_date . ' ' . $bookingpress_appointment_end_time;

				if('d' != $bookingpress_appointment_data['bookingpress_service_duration_unit']) {
					$bookingpress_tmp_start_time = new DateTime($start_datetime_string);
					$bookingpress_tmp_end_time = new DateTime($end_datetime_string);
									...
									if (isset($repeater['repeater_data']) && !empty($repeater['repeater_data'])) {
										foreach ($repeater['repeater_data'] as $guest) {
											...
											foreach ($guest['guest_data'] as $field) {
												...
												if( $field_name == 'Signature' && !empty( $field_value )){
													$field_value = '<img src="' . esc_url( $field_value ) . '" />';
												} else if( $field_name == 'Signature' && empty( $field_value )){	
													continue;
												}									
				...
				if(!empty($bookingpress_appointment_custom_fields_meta_values)){
					foreach($bookingpress_appointment_custom_fields_meta_values as $k2 => $v2) {
						...
						if(!empty($bookingpress_field_data) && $bookingpress_field_data->bookingpress_field_type == 'signature')
						{
							$v2 = '<img src="' . esc_url( $v2 ) . '" />';
						}
