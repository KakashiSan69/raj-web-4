		...
		*/
		...
		function bookingpress_validate_submitted_booking_form_func( $payment_gateway, $posted_data ) {
			...
			if ( ! empty( $posted_data ) && ! empty( $payment_gateway ) && empty($bookingpress_appointment_data['cart_items']) && $bookingpress_add_single_entry ) {
				...
				$all_fields = $wpdb->get_results( "SELECT bookingpress_field_meta_key FROM {$tbl_bookingpress_form_fields} WHERE bookingpress_field_type = 'signature' and bookingpress_field_is_hide = 0", ARRAY_A); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_form_fields is a table name. false alarm
				
				if(!empty($entry_id) && !empty($all_fields) && !empty($bookingpress_appointment_form_fields_data['form_fields'])){
					foreach($all_fields as $signature_field_key){
						
						$sign_key = $signature_field_key["bookingpress_field_meta_key"];
						if(isset($bookingpress_appointment_form_fields_data['form_fields'][$sign_key]) && !empty($bookingpress_appointment_form_fields_data['form_fields'][$sign_key])){
							$base64_string = $bookingpress_appointment_form_fields_data['form_fields'][$sign_key];


							if (preg_match('/^data:image\/(\w+);base64,/', $base64_string, $type)) {
								$image_type = $type[1]; // e.g., png, jpg, gif
								$base64_string = substr($base64_string, strpos($base64_string, ',') + 1);
								$base64_string = str_replace(' ', '+', $base64_string); // replace spaces if any
								$image_data = base64_decode($base64_string);
								if ($image_data !== false) {
									$output_file_name = $entry_id ."_".$sign_key.".".$image_type;
									$output_file = BOOKINGPRESS_PRO_UPLOAD_DIR . "/" .$output_file_name;
									file_put_contents($output_file, $image_data);
									$output_url = BOOKINGPRESS_PRO_UPLOAD_URL . "/" .$output_file_name;
									$bookingpress_appointment_form_fields_data['form_fields'][$sign_key] = $output_url;
		...
		public function bookingpress_confirm_booking( $entry_id, $payment_gateway_data, $payment_status, $transaction_id_field = '', $payment_amount_field = '', $is_front = 2, $is_cart_order = 0, $payment_currency_field = '' ) {
			...
			if($bookingpress_booked_appointment_id > 0){
				...
				if (!empty($payment_gateway) && $payment_gateway == 'on-site' && '' == $transaction_id_field ) {
				...
				if($bookingpress_entry_id){
					...
					if( $bookingpress_is_cart ){
						$entry_data = $wpdb->get_row( $wpdb->prepare( "SELECT bookingpress_coupon_details,SUM(bookingpress_coupon_discount_amount) as total_discounted_amount FROM {$tbl_bookingpress_entries} WHERE bookingpress_order_id = %d", $bookingpress_order_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_entries is a table name. false alarm
						if( !empty( $entry_data ) ){
							$bookingpress_coupon_details                = $entry_data['bookingpress_coupon_details'];
							$bookingpress_coupon_discounted_amount 		= $entry_data['total_discounted_amount'];
				...
				if($bookingpress_is_cart || $bookingpress_is_group_order){		
					...
					$bookingpress_payment_log_update_data['bookingpress_payment_adjustment_status'] = 2;
            ...
            do_action( 'bookingpress_other_debug_log_entry', 'appointment_debug_logs', 'Booking form confirm booking data -- transaction id', 'bookingpress_submit_booking_request', $transaction_id, $bookingpress_other_debug_log_id );
			...
			if ( ! empty( $entry_id ) && empty($is_cart_order) ) {
				...
				if ( ! empty( $entry_data ) ) {
					...
					do_action( 'bookingpress_other_debug_log_entry', 'appointment_debug_logs', 'Booking form confirm booking data -- appointment_booking_fields', 'bookingpress_submit_booking_request', $appointment_booking_fields, $bookingpress_other_debug_log_id );
					...
					if( !empty( $payment_gateway_data ) && 'woocommerce' != $bookingpress_payment_gateway ){
						...
						if( floatval( $bookingpress_paid_amount ) != floatval( $paid_amount ) ){
							...
							if( defined('REST_REQUEST') && $this->bookingpress_validate_token() ){
								return new WP_Error(
									400,
									__( 'Amount mismatched', 'bookingpress-appointment-booking' ).' '. $suspicious_data
								);
							}
						...
						if( !empty( $payment_currency_field ) && !empty( $payment_gateway_data[ $payment_currency_field ] ) && !preg_match( '/\|/', $payment_currency_field ) ){
							...
							if( strtolower( trim( $payment_gateway_data[ $payment_currency_field ] ) ) != strtolower( trim( $bookingpress_service_currency ) ) ){
								...
								if( defined('REST_REQUEST') && $this->bookingpress_validate_token() ){
									return new WP_Error(
										400,
										__( 'Currency mismatched', 'bookingpress-appointment-booking' ).' ' . $suspicious_data
									);
								}
					...
					if( 0 < $double_book_entry_id ){
						if( defined('REST_REQUEST') && $this->bookingpress_validate_token() ){
							return new WP_Error(
								409,
								__( 'Duplicate Appoitment data already exists', 'bookingpress-appointment-booking')
							);
						}
						status_header( 409 );
					...
					if( !empty( $prevent_booking ) && !empty( $prevent_booking['prevent_validation_process'] ) && true == $prevent_booking['prevent_validation_process'] ){
						...
						if( defined('REST_REQUEST') && $this->bookingpress_validate_token() ){
							return new WP_Error(
								409,
								$appointment_prevent_reason['message']
							);
						}
					...
					do_action( 'bookingpress_other_debug_log_entry', 'appointment_debug_logs', 'Booking form confirm booking data -- appointment_booking_fields before insert', 'bookingpress_submit_booking_request', $appointment_booking_fields, $bookingpress_other_debug_log_id );
					...
					if ( ! empty( $bookingpress_coupon_details ) ) {
						...
						if ( ! empty( $bookingpress_coupon_data ) && is_array( $bookingpress_coupon_data ) ) {
							$coupon_id = !empty( $bookingpress_coupon_data['coupon_data']['bookingpress_coupon_id'] ) ? $bookingpress_coupon_data['coupon_data']['bookingpress_coupon_id'] : ( !empty( $bookingpress_coupon_data['bookingpress_coupon_id'] ) ? $bookingpress_coupon_data['bookingpress_coupon_id'] : '' );
							$bookingpress_coupons->bookingpress_update_coupon_usage_counter( $coupon_id );
						}
					}
					...
					if ( ! empty( $inserted_booking_id ) ) {
						...
						if(isset($bookingpress_complete_payment_url_selection) && $bookingpress_complete_payment_url_selection == 'mark_as_pending'){
						...
						if(isset($bookingpress_complete_payment_url_selection) && $bookingpress_complete_payment_url_selection == 'mark_as_paid'){
							$payment_log_data['bookingpress_payment_status'] = 1;
							$payment_log_data['bookingpress_mark_as_paid'] = 1;
						}
						//----------------------------------------------
				...
				if ( ! empty( $entry_data ) ) {
					...
					$total_coupon_discounted_amount = 0;
					$coupon_data = $redeemed_coupon_details = array();
					...
					foreach($entry_data as $k => $v){
						...
						$total_coupon_discounted_amount = $total_coupon_discounted_amount + $bookingpress_coupon_discounted_amount;


						$bookingpress_coupon_details_temp = $bookingpress_coupon_details;
						if (is_string($bookingpress_coupon_details)) {
							$bookingpress_coupon_details_temp = json_decode($bookingpress_coupon_details, true);

							if(empty($redeemed_coupon_details) && !empty($bookingpress_coupon_details_temp['coupon_data'])){
								$redeemed_coupon_details = $bookingpress_coupon_details;
							}								
						}

						$coupon_data[] = $bookingpress_coupon_details_temp;
						...
						$prevent_booking = $BookingPress->bookingpress_is_appointment_booked( $appointment_sid, $appointment_date, $appointment_stime, $appointment_etime, 0, true, $posted_data, $v );
					...
					if ( ! empty( $bookingpress_coupon_details ) ) {
						...
						if ( ! empty( $bookingpress_coupon_data ) && is_array( $bookingpress_coupon_data ) ) {
							...
							if($coupon_id != '') {
								...
								$redeemed_coupon_details = json_decode( $redeemed_coupon_details, true );
								$coupon_id = !empty($redeemed_coupon_details['coupon_data']['bookingpress_coupon_id']) ? $redeemed_coupon_details['coupon_data']['bookingpress_coupon_id'] :'';							
								if($coupon_id != '') {
									$bookingpress_coupons->bookingpress_update_coupon_usage_counter( $coupon_id );
								} 
							}
					...
					if ( ! empty( $bookingpress_inserted_appointment_ids ) ) {
						...
						$payment_coupon_data=  $coupon_data;
						$payment_total_coupon_discounted_amount = $total_coupon_discounted_amount;
						...
						$payment_log_data = array(
							...
							'bookingpress_payment_currency'        => $entry_details['bookingpress_service_currency'],
							'bookingpress_coupon_details'          => wp_json_encode($payment_coupon_data),
							'bookingpress_coupon_discount_amount'  => $payment_total_coupon_discounted_amount,
						...
						/* Condition add if payment done with mark as pending then payment status consider as '2' */
						$bookingpress_complete_payment_url_selection = isset($entry_details['bookingpress_complete_payment_url_selection']) ? $entry_details['bookingpress_complete_payment_url_selection'] : '';
						if($bookingpress_complete_payment_url_selection == 'mark_as_pending'){
							$payment_log_data['bookingpress_payment_status'] = 2;
						}

						/* Condition add if payment done with deposit then payment status consider as '4' */
						//----------------------------------------------
						if(0 < $bookingpress_due_amount && $bookingpress_deposit_amount > 0){
							$payment_log_data['bookingpress_payment_status'] = 4;
							$payment_log_data['bookingpress_mark_as_paid'] = 0;
						}
						//----------------------------------------------
						...
						foreach($bookingpress_inserted_appointment_ids as $k2 => $v2){
							...
							if($is_staff_active == 1 && isset($payment_log_data['bookingpress_is_cart']) && $payment_log_data['bookingpress_is_cart'] == '1'){
								...
								$bookingpress_send_multiple_staff_notification_arr = array('bookingpress_send_multiple_staff_notification'=>false,'bookingpress_send_email_staff_ids'=>$bookingpress_send_email_staff_ids);
								$bookingpress_send_multiple_staff_notification_arr = apply_filters('bookingpress_after_appointment_booking_send_multiple_staff_notification',$bookingpress_send_multiple_staff_notification_arr,$v2,$bookingpress_send_email_staff_ids); 

								$bookingpress_send_multiple_staff_notification = (isset($bookingpress_send_multiple_staff_notification_arr['bookingpress_send_multiple_staff_notification']))?$bookingpress_send_multiple_staff_notification_arr['bookingpress_send_multiple_staff_notification']:false;

								$bookingpress_send_email_multi_staff_ids = (isset($bookingpress_send_multiple_staff_notification_arr['bookingpress_send_email_staff_ids']))?$bookingpress_send_multiple_staff_notification_arr['bookingpress_send_email_staff_ids']:array();


								if((!empty($staff_member_id) && !in_array($staff_member_id, $bookingpress_send_email_staff_ids)) || $bookingpress_send_multiple_staff_notification){
									...
									if(!empty($bookingpress_send_email_multi_staff_ids) && is_array($bookingpress_send_email_multi_staff_ids)){
										$bookingpress_send_email_staff_ids = array_merge($bookingpress_send_email_staff_ids, $bookingpress_send_email_multi_staff_ids); 
									}
								}
			...
			if ( ! empty( $bookingpress_appointment_details ) && empty($bookingpress_appointment_details['cart_items']) ) {
				...
				if(!empty($payment_gateway) && $payment_gateway != "on-site" && $bookingpress_deposit_payment->bookingpress_check_deposit_payment_module_activation() && !empty($bookingpress_appointment_details['bookingpress_deposit_payment_method']) && ($bookingpress_appointment_details['bookingpress_deposit_payment_method'] == "deposit_or_full_price") && empty($bookingpress_appointment_details['cart_items']) ){
					...
					if($bookingpress_deposit_type == "percentage"){
						...
						$bookingpress_deposit_due_amt = $total_payable_amount - (float)$bookingpress_deposit_amt;
		...
		function bookingpress_insert_customer_field_data( $bookingpress_customer_id, $appointment_field_data ) {
			...
			foreach ( $bookingpress_form_fields as $bookingpress_form_field_key => $bookingpress_form_field_val ) {
				...
				} else if( ! $bookingpress_update_customer_meta && '1' != $bookingpress_form_field_val['bookingpress_field_is_default'] ){
					$is_customer_field = $wpdb->get_var( $wpdb->prepare( "SELECT bookingpress_is_customer_field FROM {$tbl_bookingpress_form_fields} WHERe bookingpress_field_meta_key = %s AND bookingpress_is_customer_field = %d", $bookingpress_form_field_val['bookingpress_field_meta_key'], 1 ) );

					if( 0 < $is_customer_field ){
						$bookingpress_field_list[] = $bookingpress_form_field_val['bookingpress_field_meta_key'];
					}
				}
			}
