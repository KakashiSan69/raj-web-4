...
if ( ! class_exists( 'BookingPressPro' ) ) {
	class BookingPressPro {
		...
		function __construct() {
			...
			$tbl_bookingpress_cron_email_notifications_logs   = $wpdb->prefix . 'bookingpress_cron_email_logs';
			...
			$tbl_bookingpress_staffmembers_special_day_breaks = $wpdb->prefix . 'bookingpress_staff_special_day_breaks';
			$tbl_bookingpress_staffmembers_activities         = $wpdb->prefix . 'bookingpress_staffmembers_activities';
			...
			add_filter( 'bookingpress_retrieve_pro_modules_timeslots', array( $this, 'bookingpress_get_default_special_days'), 12, 7 );
			add_filter( 'bpa_calculate_default_sp_day_workdata', array( $this, 'bookingpress_calculate_default_spday_workdata'), 10, 2 );

			add_filter( 'bookingpress_retrieve_pro_modules_timeslots', array( $this, 'bookingpress_get_pro_default_workhours'), 13, 7);
				...
				add_filter( 'plugin_action_links', array( $this, 'bookingpress_modify_add_on_plugins'), 10, 2 );
				if($pagenow == "plugins.php"){
					add_action('admin_footer', array( $this, 'bookingpress_activation_confirmation' ));
				}

				add_action( 'activate_bookingpress-multiservice-booking/bookingpress-multiservice-booking.php', array( $this, 'bookingpress_deactivate_multistaff') );
				add_action( 'activate_bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php', array( $this, 'bookingpress_deactivate_multi_service') );
			}

		}

		function bookingpress_deactivate_multistaff(){
			if( is_plugin_active( 'bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php' ) ){
				deactivate_plugins( plugin_basename( WP_PLUGIN_DIR.'/bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php' ) );
			}
		}

		function bookingpress_deactivate_multi_service(){
			if( is_plugin_active( 'bookingpress-multiservice-booking/bookingpress-multiservice-booking.php' ) ){
				deactivate_plugins( plugin_basename( WP_PLUGIN_DIR.'/bookingpress-multiservice-booking/bookingpress-multiservice-booking.php' ) );
			}
		}

		function bookingpress_activation_confirmation(){
			if( file_exists( WP_PLUGIN_DIR . '/bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php') && file_exists( WP_PLUGIN_DIR .'/bookingpress-multiservice-booking/bookingpress-multiservice-booking.php') ){
				?>
				<style type="text/css">
					.bpa-pro-deactivate-form-active .bpa-addon-deactivate-form-bg {background: rgba( 0, 0, 0, .5 );position: fixed;top: 0;left: 0;width: 100%;height: 100%; z-index: 99;}
					.bpa-pro-deactivate-form-active .bpapro-activate-form{position: absolute;bottom: 30px;left: 0;max-width: 500px;min-width: 360px;background: #fff;white-space: normal; width: 400px; }
					.bpapro-activate-form-wrapper{ display:none; position: relative; z-index: 999;}
					.bpa-pro-deactivate-form-active .bpapro-activate-form-wrapper{ display:inline-block; }
					.bpa-addon-deactivate-confirm-head{ padding: 10px; background: #12D488; }
					.bpa-addon-deactivate-confirm-head svg{ float:left; width: 20px; height:20px; margin-right:10px; }
					.bpa-addon-deactivate-confirm-head p{ display: inline-block; margin:0px !important; color: #fff; font-weight: 600; }
					.bpa-addon-deactivate-form-body{ padding: 15px 20px 10px; border-bottom: 1px solid #DCDCDE; }
					.bpa-addon-deactivate-form-body p{ font-size: 14px; }
					.bpa-addon-deactivate-form-footer{ padding: 10px 10px 15px; float:left; width:100%; box-sizing: border-box; }
					.bpa-addon-deactivate-form-footer button{ cursor:pointer; float:right; margin-right: 10px; }
					.bpa-pro-deactivate-btn-cancel:hover, .bpa-pro-deactivate-btn-cancel{ color: #727E95; background: #fff; border-color: #727E95; }
					.bpa-pro-deactivate-btn-submit{ background: #12D488; border-color: transparent; color: #fff; margin: 0 5px 0 0 !important;  }
				</style>
				<div class="bpa-addon-deactivate-form-bg"></div>
				<?php
				$html  = '<div class="bpa-addon-deactivate-confirm-head"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 0 24 24" width="20px" fill="#fff"><path d="M4.47 21h15.06c1.54 0 2.5-1.67 1.73-3L13.73 4.99c-.77-1.33-2.69-1.33-3.46 0L2.74 18c-.77 1.33.19 3 1.73 3zM12 14c-.55 0-1-.45-1-1v-2c0-.55.45-1 1-1s1 .45 1 1v2c0 .55-.45 1-1 1zm1 4h-2v-2h2v2z"/></svg><p>'. esc_html__( 'BookingPress Multi Staff Add-on activation', 'bookingpress-appointment-booking' ) .'</div>';
				$html .= '<div class="bpa-addon-deactivate-form-body">';
					$html .= '<p>'.sprintf( esc_html__('You are already using Multi service booking add-on on your website. If you activate the Multi-staff add-on, it will automatically deactivate the Multi service booking add-on. %s Would you like to proceed?', 'bookingpress-appointment-booking'), '<br/><br/>' ).'</p>';
				$html .= '</div>';
				$html .= '<div class="bpa-addon-deactivate-form-footer">';
					$html .= '<p>';
						$html .= '<button type="button" id="bpa-addon-deactivate-submit-btn-multi-staff" class="bpa-deactivate-btn bpa-pro-deactivate-btn-submit">'. esc_html__( 'Proceed', 'bookingpress-appointment-booking'). '</button>';
						$html .= '<button type="button" id="bpa-addon-deactivate-cancel-btn-multi-staff" class="bpa-deactivate-btn bpa-pro-deactivate-btn-cancel">'. esc_html__( 'Cancel', 'bookingpress-appointment-booking'). '</button>';
					$html .= '</p>';
				$html .= '</div>';

				$html2  = '<div class="bpa-addon-deactivate-confirm-head"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 0 24 24" width="20px" fill="#fff"><path d="M4.47 21h15.06c1.54 0 2.5-1.67 1.73-3L13.73 4.99c-.77-1.33-2.69-1.33-3.46 0L2.74 18c-.77 1.33.19 3 1.73 3zM12 14c-.55 0-1-.45-1-1v-2c0-.55.45-1 1-1s1 .45 1 1v2c0 .55-.45 1-1 1zm1 4h-2v-2h2v2z"/></svg><p>'. esc_html__( 'BookingPress Multi Service Booking Add-on activation', 'bookingpress-appointment-booking' ) .'</div>';
				$html2 .= '<div class="bpa-addon-deactivate-form-body">';
					$html2 .= '<p>'.sprintf( esc_html__('You are using Multi-staff add-on on your website. If you activate the Multi service booking Add-on, it will automatically deactivate the Multi-staff add-on. %s Would you like to proceed?', 'bookingpress-appointment-booking'), '<br/><br/>' ).'</p>';
				$html2 .= '</div>';
				$html2 .= '<div class="bpa-addon-deactivate-form-footer">';
					$html2 .= '<p>';
						$html2 .= '<button type="button" id="bpa-addon-deactivate-submit-btn-multi-service" class="bpa-deactivate-btn bpa-pro-deactivate-btn-submit">'. esc_html__( 'Proceed', 'bookingpress-appointment-booking'). '</button>';
						$html2 .= '<button type="button" id="bpa-addon-deactivate-cancel-btn-multi-service" class="bpa-deactivate-btn bpa-pro-deactivate-btn-cancel">'. esc_html__( 'Cancel', 'bookingpress-appointment-booking'). '</button>';
					$html2 .= '</p>';
				$html2 .= '</div>';
				?>
				<script type="text/javascript">
					jQuery(document).ready(function($){
						let bpa_multistaff_activate_link = $("#bpa-activate-link-<?php echo esc_attr('bookingpress-multistaff'); ?>");
						let bpa_multistaff_formContainer = $("#bpa-activate-form-<?php echo esc_attr('bookingpress-multistaff'); ?>");

						jQuery( bpa_multistaff_activate_link ).attr('onclick', "javascript:event.preventDefault();");
						jQuery( bpa_multistaff_activate_link ).on('click', function(){
							jQuery('body').toggleClass( 'bpa-pro-deactivate-form-active' );
							bpa_multistaff_formContainer.show( {complete: function(){
                                var offset = bpa_multistaff_formContainer.offset();
                                if( offset.top < 50) {
                                    $(this).parent().css('top', (50 - offset.top) + 'px')
                                }
                                jQuery('html,body').animate({ scrollTop: Math.max(0, offset.top - 350) });
                            }} );
							bpa_multistaff_formContainer.html( '<?php echo $html; //phpcs:ignore ?>' );
							
							jQuery(bpa_multistaff_formContainer).on('click','#bpa-addon-deactivate-cancel-btn-multi-staff', function(){
								bpa_multistaff_formContainer.fadeOut(); 
                                $('body').removeClass('bpa-pro-deactivate-form-active');
							});

							jQuery(bpa_multistaff_formContainer).on('click','#bpa-addon-deactivate-submit-btn-multi-staff', function(){
								window.location.href = jQuery( bpa_multistaff_activate_link ).attr('href');
							});

							jQuery('.bpa-addon-deactivate-form-bg').click(function(){
								bpa_multistaff_formContainer.fadeOut(); 
								$('body').removeClass('bpa-pro-deactivate-form-active');
							});
						});
						...
						var bpa_multiservice_activate_link = $("#bpa-activate-link-<?php echo esc_attr('bookingpress-multiservice-booking'); ?>");
						let bpa_multiservice_formContainer = $("#bpa-activate-form-<?php echo esc_attr('bookingpress-multiservice-booking'); ?>");

						jQuery( bpa_multiservice_activate_link ).attr('onclick', "javascript:event.preventDefault();");
						jQuery( bpa_multiservice_activate_link ).on('click', function(){
							jQuery('body').toggleClass( 'bpa-pro-deactivate-form-active' );
							bpa_multiservice_formContainer.show( {complete: function(){
                                var offset = bpa_multiservice_formContainer.offset();
                                if( offset.top < 50) {
                                    $(this).parent().css('top', (50 - offset.top) + 'px')
                                }
                                jQuery('html,body').animate({ scrollTop: Math.max(0, offset.top - 350) });
                            }} );
							bpa_multiservice_formContainer.html( '<?php echo $html2; //phpcs:ignore ?>' );

							jQuery(bpa_multiservice_formContainer).on('click','#bpa-addon-deactivate-cancel-btn-multi-service', function(){
								bpa_multiservice_formContainer.fadeOut(); 
                                $('body').removeClass('bpa-pro-deactivate-form-active');
							});

							jQuery(bpa_multiservice_formContainer).on('click','#bpa-addon-deactivate-submit-btn-multi-service', function(){
								window.location.href = jQuery( bpa_multiservice_activate_link ).attr('href');
							});
						});
		...
		function bookingpress_modify_add_on_plugins( $links, $file ){

			if( file_exists( WP_PLUGIN_DIR . '/bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php') && file_exists( WP_PLUGIN_DIR .'/bookingpress-multiservice-booking/bookingpress-multiservice-booking.php') ){

				if( 'bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php' == $file ){

					if( isset( $links['activate'] ) && is_plugin_active( 'bookingpress-multiservice-booking/bookingpress-multiservice-booking.php' ) ){
						$activation_link = $links['activate'];
						$activation_link   = str_replace(
							'<a',
							'<div class="bpapro-activate-form-wrapper">
								<span class="bpapro-activate-form" id="bpa-activate-form-' . esc_attr('bookingpress-multistaff') . '"></span>
							</div><a id="bpa-activate-link-' . esc_attr('bookingpress-multistaff') . '"',
							$activation_link
						);
				...
				} else if( 'bookingpress-multiservice-booking/bookingpress-multiservice-booking.php' == $file ){
					if( isset( $links['activate'] ) && is_plugin_active( 'bookingpress-multi-staffmembers/bookingpress-multi-staffmembers.php' ) ){
						$activation_link = $links['activate'];
						$activation_link   = str_replace(
							'<a',
							'<div class="bpapro-activate-form-wrapper">
								<span class="bpapro-activate-form" id="bpa-activate-form-' . esc_attr('bookingpress-multiservice-booking') . '"></span>
							</div><a id="bpa-activate-link-' . esc_attr('bookingpress-multiservice-booking') . '"',
							$activation_link
						);
		...
		function bookingpress_admin_license_notices(){
			...
			if( isset( $check_lite_update_notice ) && 1 == $check_lite_update_notice ){
				
				/** recheck the lite version first */
				$bpa_recheck_version = $this->bpa_pro_force_check_for_plugin_update();
				...
				// show admin notice
				if( true === $bpa_recheck_version ){
					?>
						<div class="bpa-pg-warning-belt-box bpa-error">
							<p class="bpa-wbb__desc">
								<span class="material-icons-round bpa-wbb__desc-icon">warning</span>
								<span class="bpa-wbb__desc-content"><?php echo $wp_plugin_update_notification; //phpcs:ignore ?></span>
							</p>
						</div>
					<?php
				}
		...
		*/
		...
		function bookingpress_modify_capability_data_func($bpa_caps){
			...
			$bpa_caps['bookingpress_appointments'][] = 'apply_for_manual_payment_adjustment';	
			$bpa_caps['bookingpress_appointments'][] = 'bookingpress_add_admin_note';	
			...
			$bpa_caps['bookingpress_myservices'][] = 'bookingpress_edit_staff_service';
		...
		function bookingpress_pro_admin_notices() {
			...
			$pro_admin_notice_arrlist = get_transient( 'bpa_pro_admin_notice_arr_list');
			if( false === $pro_admin_notice_arrlist ){
				...
				set_transient( 'bpa_pro_admin_notice_arr_list', $addon_list_arr, (2 * DAY_IN_SECONDS) );
			} else {
				$addon_list_arr = get_transient( 'bpa_pro_admin_notice_arr_list' );
			}
				...
				if(!empty($bookingpress_styling_options)){
					...
					if($booking_form_sequence == "service"){
						$bookingpress_customize_fields = array(
							...
							'bookingpress_setting_value' => json_encode( array('service_selection','staff_selection', 'datetime_selection') ),
							'bookingpress_setting_type' => 'booking_form',
						);
						...
						$bookingpress_customize_fields = array(
							'bookingpress_setting_name' => 'bookingpress_form_sequance',
							'bookingpress_setting_value' => json_encode( array( 'staff_selection', 'service_selection', 'datetime_selection' ) ),
							'bookingpress_setting_type' => 'booking_form',
						);
						$wpdb->update($tbl_bookingpress_customize_settings, $bookingpress_customize_fields, array( 'bookingpress_setting_name' => 'bookingpress_form_sequance','bookingpress_setting_type' => 'booking_form') );
					}
		...
		function bookingpress_admin_vue_data_variable_script_func() {
			global $bookingpress_coupons,$bookingpress_pro_staff_members,$bookingpress_global_options,$BookingPressPro,$BookingPress,$bookingpress_pro_settings, $bookingpress_roles;
			...
			if ( $BookingPressPro->bookingpress_check_user_role( 'bookingpress-staffmember' ) || (!empty($custom_role) && $this->bookingpress_check_user_role( $custom_role )) ) {
		...
		function bookingpress_pro_capabilities() {
			$cap = array(
				...
				'bookingpress_import_customers'			   => esc_html__( 'Import Customer', 'bookingpress-appointment-booking'),
				...
				'bookingpress_edit_services'               => esc_html__( 'Edit Services', 'bookingpress-appointment-booking' ),
				...
				'bookingpress_edit_staff_service'          => esc_html__( 'Edit Staffmember service', 'bookingpress-appointment-booking' ),
				...
				'bookingpress_notification_settigs'		   => esc_html__( 'Manage Notification Settings', 'bookingpress-appointment-booking' ),
				'bookingpress_workhours_settigs'		   => esc_html__( 'Manage Company Working Hours', 'bookingpress-appointment-booking' ),
				'bookingpress_daysoff_settigs'			   => esc_html__( 'Manage Company Holidays', 'bookingpress-appointment-booking' ),
				'bookingpress_special_day_settigs'		   => esc_html__( 'Manage Company Special Days', 'bookingpress-appointment-booking' ),
				'bookingpress_payment_settings'			   => esc_html__( 'Manage Payment Settings', 'bookingpress-appointment-booking' ),
				'bookingpress_messages_settings'		   => esc_html__( 'Manage Messages Settings', 'bookingpress-appointment-booking' ),
				'bookingpress_license'					   => esc_html__( 'Manage License Settings', 'bookingpress-appointment-booking' ),
				'bookingpress_debug_log_settings'		   => esc_html__( 'Manage Debug Log Settings', 'bookingpress-appointment-booking' ),
				'share_generated_url'					   => esc_html__( 'Allow to generate URL for appointments', 'bookingpress-appointment-booking' ),
				'change_staffmember_status'				   => esc_html__( 'Change Staffmember Status', 'bookingpress-appointment-booking' ),
				'edit_staffmember_details'				   => esc_html__( 'Edit Staff member', 'bookingpress-appointment-booking'),
				'get_staffmember_shift_managment'		   => esc_html__( 'Manage Staff Shift', 'bookingpress-appointment-booking' ),
				'bookingpress_delete_staffmembers'		   => esc_html__( 'Delete Staff Member', 'bookingpress-appointment-booking' )
			);
		...
		function bookingpress_pro_menu() {
			...
			if ( $this->bookingpress_check_user_role( 'bookingpress-staffmember' ) && ( ! $bookingpress_pro_staff_members->bookingpress_check_staffmember_module_activation() || $bookingpress_pro_staff_members->bookingpress_current_login_staffmember_status() != 1 ) ) {
				...
				if ( ! current_user_can( 'administrator' )) {
					if(!current_user_can( 'add_staffmembers_details' )){
		...
		function bookingpress_modify_header_content_func( $bookingpress_header_file_url,$from_header = 0 ) {
			global $bookingpress_coupons,$bookingpress_slugs, $bookingpress_pro_staff_members,$BookingPress, $bookingpress_roles;
			...
			$bookingpress_new_slugs = apply_filters("bookingpress_new_slugs", $bookingpress_new_slugs);
			...
			if ( ! empty( $_REQUEST['page'] ) && $_REQUEST['page'] == 'bookingpress_coupons' && ! $bookingpress_coupons->
				...
				if ( $this->bookingpress_check_user_role( 'bookingpress-staffmember' ) || (!empty($custom_role) && $this->bookingpress_check_user_role( $custom_role )) ) {
		...
		public function install_bookingpress_lite() {
			...
			if ( ! file_exists( WP_PLUGIN_DIR . '/bookingpress-appointment-booking/bookingpress-appointment-booking.php' ) ) {
				...
				else
				{
					...
					$package_data = $BookingPressPro->bpa_pro_force_check_for_plugin_update( ['dwlurl'] );
		...
		public static function bpa_pro_force_check_for_plugin_update( $param = [], $force_update = false ){
			global $bpa_plugin_slug, $wp_version;

			$bookingpress_plugin_version = '';
			if( file_exists( WP_PLUGIN_DIR . '/bookingpress-appointment-booking/bookingpress-appointment-booking.php' ) ){
				...
				$bookingpress_plugin_version 	  = $bookingpress_plugin_data['Version'];
			}
	
			$bpa_api_url = 'https://www.arpluginshop.com';
			$args = array(
				'slug' => 'bookingpress-appointment-booking',
			);
			...
			$request_string = array(
				'body' => array(
					...
					'api-key' => md5( BOOKINGPRESS_PRO_HOME_URL ),
					'is_update' => $force_update
				),
				'sslverify' => false,
				'user-agent' => 'BKPLITE-WordPress/'.$wp_version.';'.BOOKINGPRESS_PRO_HOME_URL
			);
			...
			if( isset( $response['access_request'] ) && !empty( $response['access_request'] ) && 'success' == $response['access_request'] ){
				if( isset( $response['access_package'] ) && !empty( $response['access_package'] ) ){
					...
					if( isset( $update_package ) && is_array( $update_package ) && !empty( $update_package ) ){
						...
						$current_version = $bookingpress_plugin_version;
						
						if( version_compare( $current_version, $version, '>=') ){
							delete_option( 'bookingpress_lite_show_update_failed_notice' );
							return false;
						}
					}
				}
			...
			if ( file_exists( WP_PLUGIN_DIR . '/bookingpress-appointment-booking/bookingpress-appointment-booking.php' ) ) {
				...
				if ( ! empty( $latest_version ) && version_compare( $bookingpress_plugin_data_version, $latest_version, '<' ) ) {
					if ( ! is_wp_error( $response ) && property_exists( $response, 'versions' ) ) {
						...
						$package_data = $BookingPressPro->bpa_pro_force_check_for_plugin_update( ['version', 'dwlurl'], true );

						if( !empty( $package_data ) && !empty( $package_data['version'] ) && version_compare( $bookingpress_plugin_data_version, $package_data['version'], '<' ) ){
				...
				global $BookingPressPro;
				return $BookingPressPro->install_bookingpress_lite();
			}
		}
		...
		function bookingpress_install_pro_plugin_data(){
			...
			$sql_table = "CREATE TABLE IF NOT EXISTS `{$tbl_bookingpress_staffmembers_special_day_breaks}`(
				`bookingpress_staffmember_special_day_break_id` bigint(11) NOT NULL AUTO_INCREMENT,
				`bookingpress_special_day_id` bigint(11) NOT NULL,
			...
			$sql_table = "CREATE TABLE IF NOT EXISTS `{$tbl_bookingpress_default_special_day}`(
				`bookingpress_special_day_id` bigint(11) NOT NULL AUTO_INCREMENT,					
				`bookingpress_special_day_start_date` datetime DEFAULT NULL,
				`bookingpress_special_day_end_date` datetime DEFAULT NULL,
				`bookingpress_special_day_start_time` time DEFAULT NULL,
				`bookingpress_special_day_end_time` time DEFAULT NULL,
			...
			$sql_table = "CREATE TABLE IF NOT EXISTS `{$tbl_bookingpress_default_special_day_breaks}`(
				`bookingpress_special_day_break_id` bigint(11) NOT NULL AUTO_INCREMENT,
				`bookingpress_special_day_id` bigint(11) NOT NULL,
				`bookingpress_special_day_break_start_time` time DEFAULT NULL,
				`bookingpress_special_day_break_end_time` time DEFAULT NULL,
				`bookingpress_created_at` timestamp DEFAULT CURRENT_TIMESTAMP,
			...
			$sql_table = "CREATE TABLE IF NOT EXISTS `{$tbl_bookingpress_staffmembers_activities}`(
				`bookingpress_staff_service_activity_id` bigint(11) NOT NULL AUTO_INCREMENT,
				`bookingpress_staffmember_id` int(11) NOT NULL,
				`bookingpress_service_id` int(11) NOT NULL,
				`bookingpress_service_price` float NOT NULL,
				`bookingpress_service_price_before` float NOT NULL,
				`bookingpress_service_custom_duration_id` int(11) NOT NULL,
				`bookingpress_created_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
				PRIMARY KEY (`bookingpress_staff_service_activity_id`)
			) {$charset_collate};";

			$bookingpress_dbtbl_create[ $tbl_bookingpress_staffmembers_activities ] = dbDelta( $sql_table );
			...
			$bookingpress_notification_enabled_at_col = $wpdb->get_results( $wpdb->prepare( "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND column_name = %s", DB_NAME, $tbl_bookingpress_notifications, 'bookingpress_notification_enabled_at') );
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error)  ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_query;
			}

			if( empty( $bookingpress_notification_enabled_at_col ) ){
				$wpdb->query( "ALTER TABLE {$tbl_bookingpress_notifications}  ADD `bookingpress_notification_enabled_at` DATETIME NULL" );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_notifications is a table name. false alarm
				if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
					$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
					$bpa_missing_columns[] = 'bookingpress_notification_enabled_at|tbl_bookingpress_notifications';
				}
			}
			...
			$wpdb->query( "ALTER TABLE {$tbl_bookingpress_payment_logs} ADD bookingpress_payment_adjustment_reason TEXT DEFAULT NULL AFTER bookingpress_due_amount" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_entries is a table name. false alarm
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
				$bpa_missing_columns[] = 'bookingpress_payment_adjustment_reason|tbl_bookingpress_payment_logs';
			}
			$wpdb->query( "ALTER TABLE {$tbl_bookingpress_payment_logs} ADD bookingpress_payment_adjustment_amount float DEFAULT 0 AFTER bookingpress_due_amount" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_entries is a table name. false alarm
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
				$bpa_missing_columns[] = 'bookingpress_payment_adjustment_amount|tbl_bookingpress_payment_logs';
			}
			$wpdb->query( "ALTER TABLE {$tbl_bookingpress_payment_logs} ADD bookingpress_payment_adjustment_tax_amount float DEFAULT 0 AFTER bookingpress_due_amount" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_entries is a table name. false alarm
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
				$bpa_missing_columns[] = 'bookingpress_payment_adjustment_tax_amount|tbl_bookingpress_payment_logs';
			}
			$wpdb->query( "ALTER TABLE {$tbl_bookingpress_payment_logs} ADD bookingpress_payment_adjustment_status float DEFAULT 0 AFTER bookingpress_due_amount" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_entries is a table name. false alarm
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
				$bpa_missing_columns[] = 'bookingpress_payment_adjustment_status|tbl_bookingpress_payment_logs';
			}
			//=====================================================================================================================================
			...
			/* Update Edit Appointment Fields */
			$wpdb->query( "ALTER TABLE {$tbl_bookingpress_appointment_bookings} ADD bookingpress_is_past_appointment_edited TINYINT(1) DEFAULT 0 AFTER bookingpress_is_edited" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_appointment_bookings is a table name. false alarm
			if( !empty( $wpdb->last_error ) && !preg_match('/^(Duplicate column name)/', $wpdb->last_error) ){
				$db_error_details[ $wpdb->last_query ] = $wpdb->last_error;
				$bpa_missing_columns[] = 'bookingpress_is_past_appointment_edited|tbl_bookingpress_appointment_bookings';
			}		
			...
			$bookingpress_db_fields = array(
				'bookingpress_setting_name'  => 'bookingpress_popup_btn_title',
				'bookingpress_setting_value' => __('Book Now','bookingpress-appointment-booking'),
				'bookingpress_setting_type'  => 'booking_form',
			);        
			$wpdb->insert($tbl_bookingpress_customize_settings, $bookingpress_db_fields);
        ...
        {	
            ...
            if (version_compare($bookingpress_pro_old_version, '5.1.1', '<') ) {
		...
		function bookingpres_install_default_customize_settings() {
			...
			$booking_form = array(
				...
				'bookingpress_form_sequance' => '["service_selection","staff_selection", "datetime_selection"]',
				...
				'fileupload_max_limit_reach_msg' => __("You've reached the maximum file limit.", 'bookingpress-appointment-booking'),
			);
			...
			$booking_form = array(
				...
				'login_form_phone_number_field_label' => esc_html__( 'Phone Number', 'bookingpress-appointment-booking' ),
				...
				'login_form_phone_number_required_field_label' => esc_html__( 'Please enter phone number', 'bookingpress-appointment-booking' ),				
				...
				'login_form_phone_number_field_placeholder' => esc_html__('Enter your phone number', 'bookingpress-appointment-booking'),
				...
				'update_password_error_message' => esc_html__('Password update failed. Double-check the details and try again.', 'bookingpress-appointment-booking'),
				...
				'auth_field' => 'email_address',
				'cancellation_reason_title' => __("Cancellation Reason","bookingpress-appointment-booking"),
			);			
		...
		function bookingpress_install_default_general_settings_data() {
			...
			$bookingpress_install_default_staffmember_settings_data = array(
				...
				'bookingpress_edit_staff_service' => 'false',
			...
			$bookingpress_message_settings = array(
				...
				'no_service_available' => __('There is no service available for booking', 'bookingpress-appointment-booking'),
                'no_staffmember_available' => __('There is no staff member available for booking', 'bookingpress-appointment-booking'),
			);
		...
		public function bookingpress_get_pro_default_workhours( $service_timings_data, $selected_service_id, $selected_date, $minimum_time_required, $service_max_capacity, $bookingpress_show_time_as_per_service_duration, $past_appointment_timeslots_flag ){

			/** check staffmember as first step date time and then service */

			$bpa_staff_timing_flag = 0;
			if( isset($_POST['appointment_data_obj']) && isset($_POST['appointment_data_obj']['final_form_sequence']) ){

				$bpa_staffwise_service_data = array();
				...
				if(  $_POST['appointment_data_obj']['final_form_sequence'][0] == 'staff_selection' &&  $_POST['appointment_data_obj']['final_form_sequence'][1] == 'datetime_selection' && $_POST['appointment_data_obj']['final_form_sequence'][2] == 'service_selection' ){
					...
					$bpa_staffwise_service_data = $_POST['appointment_data_obj']['bpa_available_staffwise_service']; //phpcs:ignore
				}
			}
		...
		public function bookingpress_get_default_special_days( $service_timings_data, $selected_service_id, $selected_date, $minimum_time_required, $service_max_capacity, $bookingpress_show_time_as_per_service_duration, $past_appointment_timeslots_flag ){
		...
		public static function uninstall() {
			...
			$bookingpress_tables = array(
				...
				$tbl_bookingpress_service_daysoff,
				$tbl_bookingpress_staffmembers_activities
			);
...
.bookingpress-booking-popup-btn{
	color: 	'. $price_button_text_content_color.';
	background-color:'.$primary_color.' !important;
