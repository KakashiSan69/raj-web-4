...
if ( ! class_exists( 'bookingpress_pro_addons' ) ) {
	class bookingpress_pro_addons Extends BookingPress_Core {
		...
		function bookingpress_activate_plugin_func() {
			...
			$addon_name = !empty( $_POST['addon_name'] ) ? sanitize_text_field( $_POST['addon_name'] ) : ''; //phpcs:ignore

			if ( ! empty( $bookingpress_activate_plugin_name ) ) {

				/** Check for the incompatible add-on confirmation */
				if( !empty( $_POST['incompatible_addons'] ) && ( empty( $_POST['skip_confirmation'] ) || 'true' != $_POST['skip_confirmation'] ) ){
					$show_confirmatio_box = false;
					$incompatible_addon_names = [];
					foreach( $_POST['incompatible_addons'] as $incompatible_addon ){
						$addon_key = $incompatible_addon['key'];
						if( is_plugin_active( $addon_key ) ){
							$show_confirmatio_box = true;
							$incompatible_addon_names[] = $incompatible_addon['name'];
						}
					}
					
					if( true == $show_confirmatio_box ){
				...
				if ( $bookingpress_activate_plugin_name == 'bookingpress-woocommerce/bookingpress-woocommerce.php' ) {
					...
					$response['title']   = esc_html__( 'Success', 'bookingpress-appointment-booking' );
					$response['msg']     = esc_html__( 'Addon Activated Successfully', 'bookingpress-appointment-booking' );
					if( !empty( $_POST['incompatible_addons'] ) ){
						foreach( $_POST['incompatible_addons'] as $incompatible_addon ){
							$addon_key = $incompatible_addon['key'];
							if( is_plugin_active( $addon_key ) ){
		...
		function bookingpress_addons_dynamic_vue_methods_func() {
			...
			?>
				...
				bookingpress_activate_plugin(bookingpress_activate_plugin_name, activate_plugin_key, addon_name, incompatible_addons = [], skip_confirmation = false ){
					...
					postdata.show_confirmation = show_confirmation;
					postdata.incompatible_addons = incompatible_addons;
					postdata.skip_confirmation = skip_confirmation;
					postdata.addon_name = addon_name;
					let args = arguments;
					postdata._wpnonce = '<?php echo esc_html( wp_create_nonce( 'bpa_wp_nonce' ) ); ?>';
					axios.post( appoint_ajax_obj.ajax_url, Qs.stringify( postdata ) )
					.then( function (response) {
						if( 'undefined' == typeof response.data.msg ){
							vm.$notify({
								title: "Error",
								message: "<?php esc_html_e( "Please activate license of BookingPress premium plugin to use BookingPress Add-on", "bookingpress-appointment-booking" ); ?>",
								type: "error",
								customClass: response.data.variant + "_notification",
								duration: <?php echo intval( $bookingpress_notification_duration ); ?>,
							});
							...
							if( 'confirmation' == response.data.variant ){
								vm.$confirm( response.data.msg, 'Warning', {
									...
									vm.is_display_activate_loader = '0'
									vm.is_disabled_activate = false;
									vm.bookingpress_activate_plugin( args[0], args[1], args[2], args[3], true );
								}).catch( () => {
									vm.is_display_activate_loader = '0'
									vm.is_disabled_activate = false;
								});
