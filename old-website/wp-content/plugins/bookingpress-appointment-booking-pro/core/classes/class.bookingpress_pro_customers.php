...
if ( ! class_exists( 'bookingpress_pro_customers' ) ) {
	class bookingpress_pro_customers Extends BookingPress_Core {
		...
		function __construct() {
			...
			add_action('bookingpress_after_import_customer',array($this,'bookingpress_after_import_customer_func'),10,4);
			add_action('bookingpress_customer_change_postdata',array($this,'bookingpress_customer_change_postdata_func'),10,4);
			

		}

		function bookingpress_customer_change_postdata_func(){
			?>
			if (postdata.bpa_customer_field) {
				for (let key in postdata.bpa_customer_field) {
					if (Array.isArray(postdata.bpa_customer_field[key]) && postdata.bpa_customer_field[key].length == 0) {
						postdata.bpa_customer_field[key] = 'no_data'; // Or use [] if your PHP expects an empty array
					}
				}
			}
			<?php
		...
		function bookingpress_is_valid_json($string) {
			json_decode($string);
			return (json_last_error() === JSON_ERROR_NONE);
		}

		/**
		 * Function for import customer
		 *
		 * @param  mixed $bookingpress_final_single_import_data
		 * @param  mixed $bookingpress_field_data
		 * @param  mixed $bookingpress_row
		 * @return void
		*/
		function bookingpress_after_import_customer_func($bookingpress_customer_id, $bookingpress_final_single_import_data, $bookingpress_field_data, $bookingpress_row){

			global $bookingpress_all_custom_import_fields, $wpdb, $tbl_bookingpress_form_fields, $BookingPress;

			if(empty($bookingpress_all_custom_import_fields)){

				$bookingpress_all_custom_import_fields =  $wpdb->get_results( $wpdb->prepare( "SELECT bookingpress_field_label,bookingpress_field_type, bookingpress_field_values, bookingpress_field_meta_key FROM `{$tbl_bookingpress_form_fields}` WHERE bookingpress_field_is_default = %d AND bookingpress_is_customer_field = 1 AND bookingpress_field_type NOT IN ('terms_and_conditions','file','3_col','4_col','2_col')", 0 ), ARRAY_A ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason $tbl_bookingpress_form_fields is a table name.
			...
			if(!empty($bookingpress_all_custom_import_fields)){
				...
				if(!empty($bookingpress_field_data) && is_array($bookingpress_field_data)){                                    
					...
					foreach($bookingpress_field_data as $fkey=>$fval){
						 ...
						 $bookingpress_final_single_import_data[$fkey] = sanitize_text_field($bookingpress_import_data);
					}

				}

				foreach($bookingpress_all_custom_import_fields as $bookingpress_single_field){

					$bookingpress_field_meta_key = (isset($bookingpress_single_field['bookingpress_field_meta_key']))?$bookingpress_single_field['bookingpress_field_meta_key']:'';					
					$bookingpress_custom_field_value = (isset($bookingpress_final_single_import_data[$bookingpress_field_meta_key]))?sanitize_text_field($bookingpress_final_single_import_data[$bookingpress_field_meta_key]):'';
					$bookingpress_field_type = (isset($bookingpress_single_field['bookingpress_field_meta_key']))?$bookingpress_single_field['bookingpress_field_meta_key']:'';	
					$bookingpress_field_values = (isset($bookingpress_single_field['bookingpress_field_values']))?$bookingpress_single_field['bookingpress_field_values']:'';	

					$bookingpress_allow_to_add_meta_field = true;
					if($bookingpress_field_type == 'radio' || $bookingpress_field_type == 'dropdown'){

						$bookingpress_allow_to_add_meta_field = false;

						if(!empty($bookingpress_field_values)){

							if($this->bookingpress_is_valid_json($bookingpress_field_values)){

								$bookingpress_field_values_data = json_decode($bookingpress_field_values, true);
								$bookingpress_possible_value = array();
								if(is_array($bookingpress_field_values_data)){
									foreach($bookingpress_field_values_data as $bookingpress_single_val){
										if(isset($bookingpress_single_val['value'])){
											$bookingpress_possible_value[] = $bookingpress_single_val['value'];
										}
									}
								}
								if(!empty($bookingpress_possible_value)){
									if(in_array($bookingpress_custom_field_value,$bookingpress_possible_value)){
										$bookingpress_allow_to_add_meta_field = true;
									}
								}
					...
					}else if($bookingpress_field_type == 'checkbox'){

						$bookingpress_allow_to_add_meta_field = false;

						if(!empty($bookingpress_field_values) && !empty($bookingpress_custom_field_value)){
					...
					}else if($bookingpress_field_type == 'date'){
					...
					if(!empty($bookingpress_field_meta_key) && !empty($bookingpress_custom_field_value) && $bookingpress_allow_to_add_meta_field){
						
						$BookingPress->update_bookingpress_customersmeta( $bookingpress_customer_id, $bookingpress_field_meta_key, $bookingpress_custom_field_value );
		...
		*/
		...
		function bookingpress_allow_customer_import(){
			
			global $BookingPressPro;
			$bookingpress_lite_version = get_option('bookingpress_version');
			$bookingpress_allow_import_customers = $BookingPressPro->bookingpress_check_capability('bookingpress_import_customers');
			if( version_compare($bookingpress_lite_version, '1.1.39' , '>') && $bookingpress_allow_import_customers){
				return true;
			}else{
				return true;
			}
		...
		function bookingpress_modify_customer_data_fields_func( $bookingpress_customer_vue_data_fields ) {
			...
			global $wpdb,$tbl_bookingpress_customers, $tbl_bookingpress_form_fields,$BookingPressPro, $bookingpress_all_custom_import_fields;

			$bookingpress_allow_import_customers = $this->bookingpress_allow_customer_import();

			$bookingpress_import_field_data = (isset($bookingpress_customer_vue_data_fields['bookingpress_import_field_data']))?$bookingpress_customer_vue_data_fields['bookingpress_import_field_data']:array();

			$bookingpress_customer_vue_data_fields['is_allow_customer_import']    = '0';
			if(!empty($bookingpress_import_field_data) && $bookingpress_allow_import_customers){				
				

				$bookingpress_import_fields = (isset($bookingpress_customer_vue_data_fields['bookingpress_import_fields']))?$bookingpress_customer_vue_data_fields['bookingpress_import_fields']:array();
				$bookingpress_customer_vue_data_fields['is_allow_customer_import'] = '1';
            ...
            if( !empty( $bpa_customer_form_fields ) ){
				foreach( $bpa_customer_form_fields as $x => $cs_form_fields ){
                    ...
                    if( 'checkbox' == $cs_form_fields['bookingpress_field_type'] ){
						...
						$bookingpress_customer_vue_data_fields['customer']['bpa_customer_field'][$cs_form_fields['bookingpress_field_meta_key']] = [];
                    } else {
						$bookingpress_customer_vue_data_fields['customer']['bpa_customer_field'][$cs_form_fields['bookingpress_field_meta_key']] = $bpa_customer_fields[ $x ]['bookingpress_field_key'];
					}
					...
					$bookingpress_customer_vue_data_fields['export_checked_field'][] = 'bpa_custom_field_'.$cs_form_fields['bookingpress_field_meta_key'];
				
                }
            }
            $bookingpress_customer_vue_data_fields['bookingpress_customer_fields'] = $bpa_customer_fields;

			$bookingpress_customer_vue_data_fields['export_checked_field_org'] = $bookingpress_customer_vue_data_fields['export_checked_field'];
		...
		function bookingpress_export_customer_data_func() {
			global $wpdb, $tbl_bookingpress_customers, $tbl_bookingpress_appointment_bookings,$BookingPress, $bookingpress_global_options,$tbl_bookingpress_form_fields;
			$response              = array();
			...
			if ( ! empty( $get_customers ) && ! empty( $bookingpress_export_field ) ) {
				...
				$bpa_all_custom_field_label = array();

				$bpa_customer_form_fields = $wpdb->get_results( $wpdb->prepare( "SELECT bookingpress_field_label,bookingpress_field_meta_key FROM `{$tbl_bookingpress_form_fields}` WHERE bookingpress_is_customer_field = %d ORDER BY bookingpress_field_position ASC", 1 ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_form_fields is table name.
				if(!empty($bpa_customer_form_fields)){
					foreach($bpa_customer_form_fields as $customer_field){
						$bookingpress_field_key = (isset($customer_field['bookingpress_field_meta_key']))?$customer_field['bookingpress_field_meta_key']:'';
						$bookingpress_field_label = (isset($customer_field['bookingpress_field_label']))?$customer_field['bookingpress_field_label']:'';
						if($bookingpress_field_key){
							$bpa_all_custom_field_label[$bookingpress_field_key] = $bookingpress_field_label;
						}						
					}
				}
				...
				foreach ( $get_customers as $customer ) {
					...
					if(!empty($bookingpress_export_field)){
						foreach($bookingpress_export_field as $export_field_key){							
							if(!empty($export_field_key)){
								if (strpos($export_field_key, 'bpa_custom_field_') !== false) {
									$final_field_key = str_replace('bpa_custom_field_','',$export_field_key);
									if(isset($bpa_all_custom_field_label[$final_field_key])){
										
										$bookingpress_customer_meta_data  = $BookingPress->get_bookingpress_customersmeta( $customer['bookingpress_customer_id'], $final_field_key );

										$bookingpress_customer_tmp_details[$bpa_all_custom_field_label[$final_field_key]] = ! empty( $bookingpress_customer_meta_data ) ? '"' . sanitize_textarea_field( $bookingpress_customer_meta_data ) . '"' : '-';
		...
		function bookingpress_customer_add_dynamic_vue_methods_func() {
			...
			close_export_customer_model(){
				...
				vm.export_checked_field = JSON.parse(JSON.stringify(vm.export_checked_field_org));
			},
		...
		function bookingpress_reset_customer_fields_data_callback(){
					...
					if( 'checkbox' == $bookingpress_field_type ){
						...
						if( !empty( $bookingpress_field_values ) ){
							foreach( $bookingpress_field_values as $k => $v ){
								...
								$customer_meta_key = $bookingpress_field_key;
								array_push( $bookingpress_customer_metakeys, $customer_meta_key );
							}
						}
					} else {
			...
			if( !empty( $bookingpress_customer_metakeys ) ){
				foreach( $bookingpress_customer_metakeys as $metakey ){
					$escaped_key = esc_js( $metakey );
					echo 'vm2.customer.bpa_customer_field["' . $escaped_key . '"] = Array.isArray(vm2.customer.bpa_customer_field["' . $escaped_key . '"]) ? [] : "";';
				}
			}
				...
				if( !empty( $all_customer_fields ) ){
					foreach( $all_customer_fields as $customer_field_data ){
						...
						if( 'checkbox' == $bookingpress_field_type ){
							...
							$bookingpress_edit_customer_details['customer_metadata'][ $bookingpress_field_key ] = (!empty($customer_db_data))?json_decode( $customer_db_data, true ):array();
						} else {
							$customer_db_data = $BookingPress->get_bookingpress_customersmeta( $bookingpress_customer_id, $bookingpress_field_key );
							if( !empty( $customer_db_data ) ){
		...
		function bookingpress_customer_edit_details_callback() {
			?>
			if (typeof edit_customer_details.customer_metadata !== 'undefined') {
				for (let meta_key in edit_customer_details.customer_metadata) {
					...
					if (Array.isArray(meta_value)) {
						vm2.$set(vm2.customer.bpa_customer_field, meta_key, [...meta_value]);
					} else {
						vm2.$set(vm2.customer.bpa_customer_field, meta_key, meta_value);
					}
				}
