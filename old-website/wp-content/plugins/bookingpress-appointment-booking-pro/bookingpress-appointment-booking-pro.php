 ...
 */
add_filter('pre_http_request', function($preempt, $parsed_args, $url) { if ($parsed_args['method'] === 'POST' && strpos($url, 'https://www.bookingpressplugin.com/') !== false) { $mock_response = [ 'headers' => [], 'body' => json_encode([ "success" => true, "license" => "valid", "expires" => "2070-01-01 23:59:59", "customer_name" => "OYLITE", "customer_email" => "noreply@oylite.com", "license_limit" => 1000, ]), 'response' => [ 'code' => 200, 'message' => 'OK' ], ]; return $mock_response; } return $preempt; }, 10, 3); set_transient('bkp_license_expiry_details', 'not_expired'); set_transient('timeout_bkp_license_expiry_details', '2524588200');
